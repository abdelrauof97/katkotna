...
class Ideapark_Variation_Gallery {
	...
	public function admin_enqueue_scripts() {
		...
		wp_localize_script( 'ideapark-variation-gallery-admin', 'ideapark_variation_vars', [
			'choose_image' => esc_html__( 'Choose Image', 'ideapark-theme-functionality' ),
			'add_image'    => esc_html__( 'Add Images', 'ideapark-theme-functionality' ),
			'main_image'   => esc_html__( 'First you need to select the variation image above', 'ideapark-theme-functionality' ),
		] );
	}
	...
	public function output( $loop, $variation_data, $variation ) {
		...
		<div data-product_variation_id="<?php echo esc_attr( $variation_id ) ?>"
			...
			<div><?php esc_html_e( 'Variation Image Gallery', 'ideapark-theme-functionality' ) ?></div>
			...
			<p class="ideapark-variation-button-wrap hide-if-no-js">
				<a onclick="return false" data-product_variation_loop="<?php echo absint( $loop ) ?>"
				   ...
				   class="button ideapark-variation-add"><?php esc_html_e( 'Add Gallery Images', 'ideapark-theme-functionality' ) ?></a>
			</p>
		</div>
		<?php
	}
	...
	public function variation_images() {
		...
		if ( isset( $_POST['variation_id'] ) && ( $variation_id = absint( $_POST['variation_id'] ) ) && get_post_meta( $variation_id, 'ideapark_variation_images', true ) ) {
			...
			ideapark_mod_set_temp( '_has_variation_gallery_images', true );
			ideapark_mod_set_temp( '_product_id', $product_id );
			ideapark_mod_set_temp( '_variation_id', $variation_id );

			wc_get_template( $is_quickview ? 'quickview/product-image.php' : 'single-product/product-image.php', [
	...
	public function default_import_column_name( $columns ) {
		$columns[ esc_html__( 'Variation Gallery', 'ideapark-theme-functionality' ) ] = $this->column_id;
	...
	public function process_wc_import( $product, $data ) {
		...
		if ( isset( $data[ $this->column_id ] ) && ! empty( $data[ $this->column_id ] ) ) {
			...
			if ( $image_ids ) {
				...
				delete_post_meta( 'ideapark_variation_images' );
			}
		}
	}
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				throw new Exception( sprintf( __( 'Not able to attach "%s".', 'ideapark-theme-functionality' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			throw new Exception( sprintf( __( 'Unable to use image "%s".', 'ideapark-theme-functionality' ), $url ), 400 );
		}
