...
class Ideapark_Attribute_Filter_Widget extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'ideapark-theme-functionality' );
		$this->widget_id          = 'ip_attribute_filter';
		$this->widget_name        = __( 'Kidz Filter by Attribute', 'ideapark-theme-functionality' );
	...
	public function form( $instance ) {
		if ( ideapark_woocommerce_on() ) {
			...
			esc_html_e( 'You need to activate and configure the "Woocommerce" and "Variation Swatches for WooCommerce" plugins', 'ideapark-theme-functionality' );
		}
	}
	...
	public function init_settings() {
		...
		$this->settings = [
			'title'      => [
				'type'  => 'text',
				'std'   => __( 'Filter by', 'ideapark-theme-functionality' ),
				'label' => __( 'Title', 'ideapark-theme-functionality' ),
			],
			'attribute'  => [
				'type'    => 'select',
				'std'     => $std_attribute,
				'label'   => __( 'Attribute', 'ideapark-theme-functionality' ),
			...
			'count'      => [
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show product counts', 'ideapark-theme-functionality' ),
			],
			'query_type' => [
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'ideapark-theme-functionality' ),
				'options' => [
					'and' => __( 'AND', 'ideapark-theme-functionality' ),
					'or'  => __( 'OR', 'ideapark-theme-functionality' ),
				],
			],
			'image_size' => [
				'type'  => 'number',
				...
				'max'   => 100,
				'std'   => 35,
				'label' => __( 'Image size', 'ideapark-theme-functionality' ),
				'class' => 'js-filter-field-image-size',
			],
			'layout'     => [
				'type'    => 'select',
				'std'     => 'block',
				'label'   => __( 'Item layout', 'ideapark-theme-functionality' ),
				'options' => [
					'block'  => __( 'Block', 'ideapark-theme-functionality' ),
					'inline' => __( 'Inline', 'ideapark-theme-functionality' ),
				],
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $with_count, $image_size, $layout ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$count_html = ( $with_count ? ' ' . apply_filters( 'woocommerce_layered_nav_count', '<span class="count">(' . absint( $count ) . ')</span>', $count, $term ) : '' );
					...
					switch ( $type ) {
						case 'color':
							...
							if ( $color && $color_secondary ) {
								$swatches = sprintf( '<span class="c-ip-attribute-filter__sw c-ip-attribute-filter__sw--color" style="background: linear-gradient(%3$s, %1$s 0%%, %1$s 50%%, %2$s 50%%, %2$s 100%%);">%s</span>', esc_attr( $color_secondary ), esc_attr( $color ), apply_filters( 'woo_variation_swatches_dual_color_gradient_angle', '-45deg', $hint ) );
							} else {
						...
						case 'button':
							$swatches = sprintf( '<span class="c-ip-attribute-filter__sw c-ip-attribute-filter__sw--button">%s</span>', esc_html( $term->name ) . $count_html );
							break;
				...
				if ( in_array( $type, [ 'button' ] ) || in_array( $type, [ 'image', 'color' ] ) && $layout == 'inline' ) {
					...
					$term_html = '<a rel="nofollow" class="c-ip-attribute-filter__link" href="' . esc_url( $link ) . '">' . $swatches . ideapark_wrap( esc_html( $term->name ) . $count_html, '<span class="c-ip-attribute-filter__title"><span>', '</span></span>' ) . '</a>';
				}
			} else {
				$link      = false;
				$term_html = '<span>' . esc_html( $term->name ) . $count_html . '</span>';
			}
...
register_widget( 'Ideapark_Attribute_Filter_Widget' );