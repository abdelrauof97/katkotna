...
class Ideapark_Importer {
	...
	public function init() {
		$this->session_fn        = IDEAPARK_THEME_UPLOAD_DIR . 'import.dat';
		$this->session_fn_start  = IDEAPARK_THEME_UPLOAD_DIR . 'import_start.dat';
	...
	function admin_menu() {
		add_theme_page( __( 'Import Demo Content', 'ideapark-theme-functionality' ), __( 'Import Demo Content', 'ideapark-theme-functionality' ), 'manage_options', 'ideapark_themes_importer_page', [
			$this,
			'importer_page'
		] );
	}
	...
	function importer_page() {
		...
		$output .= '<div class="ip-import-block">';
		if ( ! empty( $themes ) ) {
			$output   .= '<p><span class="subheader">' . __( 'Select the demo site you want to import: ', 'ideapark-theme-functionality' ) . '</span></p>';
			...
			foreach ( $themes as $name => $theme ) {
				$preview_button = '';
				if ( ! empty( $theme['url'] ) ) {
					$preview_button = '<a class="ip-demo-preview" href="' . esc_url( $theme['url'] ) . '" target="_blank">' . __( 'Preview Demo', 'ideapark-theme-functionality' ) . '</a>';
				}
				if ( ! empty( $theme['screenshot'] ) ) {
					$output .= '<li class="ip-demo" data-revslider="' . ( ! empty( $theme['revslider'] ) ? 'yes' : 'no' ) . '"><label><img class="ip-screenshot" alt="' . esc_attr( $theme['title'] ) . '" src="' . esc_url( $theme['screenshot'] ) . '" /><input class="ip-import-demo" type="radio" name="import_demo" value="' . esc_attr( $name ) . '" ' . ( $is_first ? 'checked' : '' ) . '/>' . esc_attr( $theme['title'] ) . '</label>' . $preview_button . '</li>';
				} else {
		...
		if ( ! empty( $themes ) || IDEAPARK_THEME_DEMO ) {
			...
			if ( ( $code = ideapark_get_purchase_code() ) && $code !== IDEAPARK_THEME_SKIP_REGISTER ) {
				...
				$output .= '<p class="submit">' . ( PHP_OS == "Darwin" || ! IDEAPARK_THEME_DEMO ? '<button class="button button-primary" ' . ( ! empty( $this->importer['steps'] ) ? ' style="display:none" ' : '' ) . ' id="ip-import-submit">' . __( 'Import', 'ideapark-theme-functionality' ) . '</button> ' : '' ) . '</p>';
				$output .= '<div class="ip-loading-progress' . ( ! empty( $this->importer['steps'] ) ? ' paused' : '' ) . '"><div class="ip-loading-bar"><div class="ip-loading-state"></div><div class="ip-loading-info">' . __( 'Progress', 'ideapark-theme-functionality' ) . ': 0%...</div></div><div class="ip-import-output">' . __( 'Prepare data...', 'ideapark-theme-functionality' ) . '</div></div>';
				if ( ! empty( $this->importer['steps'] ) ) {
					$percent = round( ( $this->importer['base']->step_done / $this->importer['base']->step_total ) * 100 );
					$output  .= '<div class="ip-import-continue">' . ( PHP_OS == "Darwin" || ! IDEAPARK_THEME_DEMO ? '<button type="button" class="button button-primary" id="ip-import-continue" data-percent="' . esc_attr( $percent ) . '">' . __( 'Continue', 'ideapark-theme-functionality' ) . '</button>&nbsp;&nbsp;&nbsp;<button type="button" class="button button-default" id="ip-import-cancel" data-percent="' . esc_attr( $percent ) . '">' . __( 'Cancel', 'ideapark-theme-functionality' ) . '</button> ' : '' ) . '</div>';
				}
				...
				$output .= sprintf( wp_kses( __( 'We recommend you to <a href="%s" target="_blank">reset data</a> & clean wp-content/uploads folder before import to prevent duplicate content.', 'ideapark-theme-functionality' ), [ 'a' => [ 'href' => [] ] ] ), esc_url( 'https://wordpress.org/plugins/wp-reset/' ) ) . '<br />';

				if ( ! ini_get( 'allow_url_fopen' ) ) {
					$output .= '<div class="error">' . esc_html__( 'Downloading remote files is prohibited in PHP (allow_url_fopen in php.ini)', 'ideapark-theme-functionality' ) . '</div>';
				} elseif ( ! isset( $_GET['download_test'] ) ) {
					$output .= '<p>' . esc_html__( 'If you have problems importing images, then run a test to get information about the error', 'ideapark-theme-functionality' ) . ': <a href="' . admin_url( 'themes.php?page=ideapark_themes_importer_page&download_test' ) . '">' . esc_html__( 'download test', 'ideapark-theme-functionality' ) . '</a></p>';
				} else {
					if ( ! defined( 'WP_LOAD_IMPORTERS' ) ) {
						define( 'WP_LOAD_IMPORTERS', true );
					}
					...
					if ( is_wp_error( $result ) ) {
						$output .= '<div class="error">' . esc_html__( 'Download test failed', 'ideapark-theme-functionality' ) . ': ' . $result->get_error_message() . '</div>';
					} else {
						$output .= '<p class="warning">' . esc_html__( 'Download test completed successfully', 'ideapark-theme-functionality' ) . '</p>';
				...
				$output .= '<p class="ideapark-theme-functionality-warning">' . esc_html__( 'Available only to registered theme', 'ideapark-theme-functionality' ) . '</p>';
				$output .= '<p><a class="button button-primary" href="' . admin_url( 'themes.php?page=ideapark_about' ) . '">' . __( 'Register theme', 'ideapark-theme-functionality' ) . '</a></p>';
				$output .= '<div class="ip-import-output"></div>';
			}
			if ( defined( 'IDEAPARK_THEME_DEMO' ) && IDEAPARK_THEME_DEMO ) {
				$output .= '<p><a href="" style="color: #F0F0F1;" onclick="return false;" id="ip-export-submit">' . __( 'Export', 'ideapark-theme-functionality' ) . '</a></p>';
			}
		} else {
			$output .= '<div>' . __( 'No themes data found.', 'ideapark-theme-functionality' ) . '</div>';
			$output .= '<div>' . __( 'Try adding this code to the beginning of the file wp-config.php:', 'ideapark-theme-functionality' ) . '</div>';
	...
	function importer() {
		...
		if ( isset( $_REQUEST['stage'] ) && $_REQUEST['stage'] == 'cancel' ) {
			...
			if ( ! empty( $_REQUEST['import_option'] ) && ! empty( $_REQUEST['import_demo'] ) ) {
				...
				if ( ! ideapark_is_dir( $theme_title_fn = $this->importer_dir . '/' . $this->demo_content_folder . '/' ) ) {
					$this->import_response( 'error', __( 'Error: Can`t open file: ', 'ideapark-theme-functionality' ) . $theme_title_fn );
				}
			...
			$sub_folders = $wp_filesystem->dirlist( IDEAPARK_THEME_UPLOAD_DIR );
			if ( $sub_folders ) {
				foreach ( $sub_folders as $sub_name => $sub_item ) {
					if ( $sub_item['type'] == 'f' ) {
						ideapark_delete_file( IDEAPARK_THEME_UPLOAD_DIR . $sub_name );
					}
				}
			}
		...
		if ( empty( $this->importer['steps'][0] ) ) {
			...
			if ( empty( $this->importer['steps'][0] ) ) {
				$this->import_response( 'error', __( 'Error: Select the data you want to import', 'ideapark-theme-functionality' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
			}
		}

		if ( empty( $this->importer['import_demo_content_folder'] ) ) {
			$this->import_response( 'error', __( 'Error: Selected data is empty', 'ideapark-theme-functionality' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
		}
		...
		switch ( $step ) {
			...
			case 'completed':
				...
				if ( ! count( $this->importer['base']->error_msg ) ) {
					...
					$message                 = '<b style="color:#444">' . __( 'Data import completed!', 'ideapark-theme-functionality' ) . '</b>' . '<p><a class="button" href="' . $permalinks_settings_url . '">' . __( 'Re-save the site permalinks! ', 'ideapark-theme-functionality' ) . '</a></p>' . '<div>' . implode( '', $this->importer['base']->error_msg ) . '</div>';
				}
				$code = 'completed';
					...
					if ( $step == 'prepare' ) {
						...
						if ( ! $_result || ! ideapark_is_file( $this->session_fn_start ) ) {
							if ( get_filesystem_method() !== 'direct' ) {
								$this->import_response( 'error', __( 'Error: No access to data folder. Try to add line "define(\'FS_METHOD\',\'direct\');" to the file wp-config.php', 'ideapark-theme-functionality' ) );
							} else {
								$this->import_response( 'error', __( 'Error: Uploads folder must be writable - ', 'ideapark-theme-functionality' ) . IDEAPARK_THEME_UPLOAD_DIR );
							}
						}
					}
				} catch ( Exception $e ) {
	...
	function import_finish() {
		...
		if ( ideapark_is_file( $fn = IDEAPARK_THEME_UPLOAD_DIR . 'demo.dat' ) ) {
			ideapark_delete_file( $fn );
		}

		if ( ideapark_is_file( $fn = IDEAPARK_THEME_UPLOAD_DIR . 'content.xml' ) ) {
			ideapark_delete_file( $fn );
		}
	}
...
//				ideapark_fpc( $theme_options_fn . '.txt', base64_decode( $theme_options_txt ) );
				...
				foreach ( $options as $mod_name => $val ) {
					...
					} elseif ( preg_match( '~^(home_product_order)~i', $mod_name ) && preg_match_all( '|\~!(.+)\~\#\~(.+)!\~|Uu', $val, $matches, PREG_SET_ORDER ) ) {
		...
		$f          = implode( '_', [ 'ideapark', 'api', 'theme', 'get', 'file' ] );
		$fn         = $f( $options_fn, $is_preliminary ? '' : $this->demo_content_folder );
	...
	function import_revslider() {
		$slides = [];
		if ( ideapark_is_file( $fn = $this->importer_dir . '/' . $this->demo_content_folder . '/revslider.txt' ) ) {
			if ( $s = ideapark_fgc( $fn ) ) {
				$e = explode( ',', $s );
				foreach ( $e as $_fn ) {
					$slides[] = trim( $_fn );
				}
		...
		if ( $slides ) {
			if ( class_exists( 'RevSlider' ) ) {
				...
				foreach ( $slides as $_slide ) {
					...
					$widgets_json = $this->demo_content_folder . '/' . $_slide;
					$f            = implode( '_', [ 'ideapark', 'api', 'theme', 'get', 'file' ] );
					$fn           = $f( $widgets_json );
				...
				if ( ! $has_errors ) {
					$message = esc_html__( 'Imported Slider Revolution successfully', 'ideapark-theme-functionality' );
				}

			} else {
				$this->importer['base']->error_msg[] = $message = __( 'The plugin Slider Revolution is not installed', 'ideapark-theme-functionality' );
			}
		} else {
			$message = __( 'There are no slides in this demo', 'ideapark-theme-functionality' );
		}
...
//		}
		...
		foreach ( $options as $name => $value ) {
			if ( is_string( $value ) && preg_match( '~https?://(localhost|parkofideas)~', $value ) && ! preg_match( '~_link~', $name ) ) {
...
//			echo "<b>REV SLIDER</b>: " . $val_orig . ' ' . $data->settings->wp->rev_slider . '<br>';
		...
		if ( isset( $data->settings ) ) {
			foreach ( $data->settings as $key => $val ) {
				...
				if ( preg_match( '~_list$~i', $key ) && is_array( $data->settings->{$key} ) ) { // ::REPEATER
					foreach ( $data->settings->{$key} as &$item ) {
						foreach ( $item as $_key => $_val ) {
							// SVG Icons
							if ( isset( $item->{$_key}->library ) && $item->{$_key}->library == 'svg' && ! empty( $item->{$_key}->value->id ) ) {
								if ( $mode ) {
									$this->_unpack_img( $item->{$_key}->value->id, $item->{$_key}->value->url );
								} else {
									$this->_pack_img( $item->{$_key}->value->id, $item->{$_key}->value->url );
								}
							}

							//Images
							if ( preg_match( '~image~i', $_key ) && isset( $item->{$_key}->url ) ) {
								if ( $mode ) {
									$this->_unpack_img( $item->{$_key}->id, $item->{$_key}->url );
								} else {
									$this->_pack_img( $item->{$_key}->id, $item->{$_key}->url );
								}
							}
						}
					}
				...
				if ( preg_match( '~image~i', $key ) && isset( $data->settings->{$key}->url ) ) {
					if ( $mode ) {
						$this->_unpack_img( $data->settings->{$key}->id, $data->settings->{$key}->url );
					} else {
						$this->_pack_img( $data->settings->{$key}->id, $data->settings->{$key}->url );
					}
				}
			}
		}
	...
	function prepere_post_meta( $mode ) {
		global $wpdb;
		$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->postmeta} WHERE meta_key = %s", $mode ? '_elementor_data_kidz' : '_elementor_data' ) );
		foreach ( $postmeta as $meta ) {
			...
			if ( $mode ) {
				...
				delete_post_meta( $meta->post_id, '_elementor_data_kidz' );
			} else {
//				update_post_meta( $meta->post_id, '_elementor_data_kidz', '' );
//				$table = _get_meta_table( 'post' );
//				$wpdb->update( $table, [ 'meta_value' => wp_json_encode( $data ) ], [ 'post_id'  => $meta->post_id,
//				                                                                      'meta_key' => '_elementor_data_kidz'
//				] );
				...
				update_post_meta( $meta->post_id, '_elementor_data_kidz', wp_slash( wp_json_encode( $data ) ) );
			}
		}
	...
	public function on_wp_import_post_meta(
		...
		foreach ( $post_meta as &$meta ) {
			if ( '_elementor_data_kidz' === $meta['key'] ) {
	...
	public function on_wxr_importer_pre_process_post_meta(
		...
		if ( '_elementor_data_kidz' === $post_meta['key'] ) {
