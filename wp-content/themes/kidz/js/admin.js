(function ($, root, undefined) {
	...
	$(document).on('tinymce-editor-init', function () {
		$('.ideapark_text_editor .wp-editor-area').each(function () {
			var tArea = $(this), id = tArea.attr('id'), input = tArea.parents('.ideapark_text_editor').prev(),
				editor = tinyMCE.get(id), content;
	...
	(function (api) {
		...
		$(document).ready(function () {
			$('#customize-controls input[type="number"][step="1"]').on('change', function (e) {
				var $this = $(this);
				var val = $this.val();
			...
			var ideapark_checklist_delete = function () {
				...
				if (delete_ajax_action && !ideapark_empty(section_id)) {
					...
					$.ajax({
						url       : ideapark_ac_vars.ajaxUrl, type: 'POST', data: {
							action: delete_ajax_action, section: section_id
			...
			$('.ideapark_checklist_add_button').on('click', function () {
				...
				if (add_ajax_action && !ideapark_empty(section_id) && $control.length) {
					...
					$.ajax({
						url       : ideapark_ac_vars.ajaxUrl, type: 'POST', data: {
							action: add_ajax_action, section: section_id
						}, success: function (result) {
							$button.removeClass('loading');
							$button.prop('disabled', false);
							if (result.name) {
		...
		api.bind('change', function (obj) {
			...
			$('#customize-theme-controls .control-section').each(function () {
		...
		api.bind('ready', function () {
			...
			api.panel('front_page_builder', function (panel) {
				panel.expanded.bind(function (isExpanded) {
					if (isExpanded && ideapark_ac_vars.frontUrl) {
						api.previewer.previewUrl.set(ideapark_ac_vars.frontUrl);
					}
				});
			});
			
			api.section('ideapark_section_contact', function (section) {
				section.expanded.bind(function (isExpanded) {
					if (isExpanded && ideapark_ac_vars.contactUrl) {
						api.previewer.previewUrl.set(ideapark_ac_vars.contactUrl);
					}
				});
			});
			...
			$('#customize-theme-controls .control-section > .accordion-section-title').on('click', function () {
			...
			$('#_customize-input-theme_font_0').select2();
			
			$('#_customize-input-theme_font_1').select2();
			
			$('#_customize-input-theme_font_2').select2();
		});
				...
				for (i in depend) {
					...
					if (control) {
						...
						for (var j in depend[i]) {
							if ((('' + depend[i][j]).indexOf('search!=') === 0 && ('' + val).indexOf(('' + depend[i][j]).substring(8)) === -1) || (('' + depend[i][j]).indexOf('search=') === 0 && ('' + val).indexOf(('' + depend[i][j]).substring(7)) !== -1) || (depend[i][j] == 'not_empty' && (val !== '' && val !== '0' && val !== 0 && val !== false)) ||
								(depend[i][j] == 'is_empty' && (val === '' || val === '0' || val === 0 || val === false)) ||
								(val !== '' && (!isNaN(depend[i][j]) ?
									val == depend[i][j] : (dep_strict ? val == depend[i][j] : ('' + val).indexOf(depend[i][j]) == 0))) || (val !== '' && ('' + depend[i][j]).charAt(0) == '^' && ('' + val).indexOf(depend[i][j].substr(1)) == -1)	// Main field value not equal to specified value - show current field
							) {
								dep_cnt++;
								break;
							}
